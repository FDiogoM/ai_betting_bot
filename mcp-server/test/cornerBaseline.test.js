...
test('a venue fallback is declared once per team, not once per rate', () => {
  const thin = profile([match('home', 6, 4), match('away', 8, 2)]);

  // Both teams fall back, so both caveats are present — and each exactly once.
  // A team's for and against rates come off the same sample, and emitting the
  // fallback per accessor put the identical sentence in twice. These caveats
  // are copied verbatim into the permanent ledger record.
  const b = cornerBaseline(thin, profile([match('away', 5, 5), match('home', 3, 7)]));

  const venueCaveats = b.caveats.filter((c) => /venue sample/i.test(c));
  assert.strictEqual(venueCaveats.length, 2,
    `expected one caveat per team, got: ${venueCaveats.join(' | ')}`);
  assert.strictEqual(new Set(venueCaveats).size, 2, 'the two caveats must not be duplicates');
});
